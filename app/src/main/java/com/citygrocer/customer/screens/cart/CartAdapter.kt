...
class CartAdapter(var context: Context,
                  ...
                  var adapterClickListener: IAdapterClickListener? = null) : RecyclerView.Adapter<CartAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, p1: Int): ViewHolder {
        var view = parent.inflate(R.layout.item_cart_grocery_staples)
    ...
    inner class ViewHolder(override val containerView: View) : RecyclerView.ViewHolder(containerView), LayoutContainer {
        fun bind(context: Context, item: Any, pos: Int) {
            ...
            if (item is ProductCatResIp) {
                tv_discount_price_staples.text = item.selling_price
        ...
        fun clickForModify(view: View?, item: ProductCatResIp) {
            if (item.selSku == null)
                return
            when (view!!.id) {
                R.id.tv_increment -> {
        ...
        private fun assignProdAmt(sellingPrice: Float, mrp: Float, savings: Float) {
            tv_discount_price_staples.apply {
                //visibility = if (sellingPrice > 0) View.VISIBLE else View.INVISIBLE
                text = CommonUtils.getRupeesSymbol(context, "%.0f".format(sellingPrice))
            }
            tv_mrp.apply {
                //visibility = if (mrp - sellingPrice > 0) View.VISIBLE else View.INVISIBLE
            ...
            if (savings > 0) {
                tv_save_price.apply {
                    ...
                    text = CommonUtils.getRupeesSymbol(context, "%.0f".format(savings) + ")")
                }
            }
        }

        private fun setCartCount(viewHolder: ViewHolder, sku: DataSku) {
            viewHolder.apply {
                tv_sku_count.text = sku.mycart.toString()
            }
        }
